...
contract DeployStrategy is Script {
    function run() external {
        ...
        if (
            keccak256(bytes(which)) == keccak256("aave") ||
            keccak256(bytes(which)) == keccak256("all")
        ) {
            AaveStrategyFacet a = new AaveStrategyFacet();
            console.log("AaveStrategyFacet", address(a));
            console.log(
                "AaveStrategyId",
                uint256(bytes32(uint256(uint160(address(a)))))
            );
        }
        if (
            keccak256(bytes(which)) == keccak256("compound") ||
            keccak256(bytes(which)) == keccak256("all")
        ) {
            ...
            console.log(
                "CompoundStrategyId",
                uint256(bytes32(uint256(uint160(address(c)))))
            );
        }
        if (
            keccak256(bytes(which)) == keccak256("morpho") ||
            keccak256(bytes(which)) == keccak256("all")
        ) {
            ...
            console.log(
                "MorphoStrategyId",
                uint256(bytes32(uint256(uint160(address(m)))))
            );
        }
