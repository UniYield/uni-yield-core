...
library LibErrors {
    ...
    error AllocationExceedsCap(bytes32 id, uint256 attemptedBps, uint256 maxBps);

    // Strategy execution
    error StrategyDepositFailed(bytes32 id);
    error StrategyWithdrawFailed(bytes32 id);
    error StrategyExitFailed(bytes32 id);
    error InsufficientLiquidity(bytes32 id, uint256 requested, uint256 available);

    // Rebalancing
    error NoRebalanceNeeded();
    error SwitchThresholdNotMet(uint256 currentBps, uint256 candidateBps, uint256 minImprovementBps);
